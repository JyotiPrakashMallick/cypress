/* AUTO-188 We will create a reminder that should be sent at a fix time and then for the same, we will verify on the dashboard. */ 
...
describe ('Create a reminder for fix time and validate on dashboard',function()
{    
    beforeEach(function (){
...
it('disable waiting room reminder and create 2 hours fixed time reminder', function(){
    ...
    cy.get(virtualWaitingRoomSelectors.waitingRoomReminder,{timeout : 5000}).should('be.visible').click()
    ...
    .should('be.visible')
    cy.log('Disable the reminder')
    cy.get(virtualWaitingRoomSelectors.disableReminder).click()
	...
	cy.log('Enable Time to send reminder')
	cy.get(remindersScreenSelectors.reminderTimeToggle).click()
	cy.log('Set time for fixed time reminder')
	cy.enterDropDownValue(remindersScreenSelectors.fixReminderTime,'03:00 PM{enter}')
...
it('Login and validate the appointment over dashboard and then change the reminders to default',function(){
    ...
    cy.get(remindersScreenSelectors.dashboardReminder).should('contain',testData.pmsSyncedPatient)
	.and('contain','2 hours')
	.and('contain','at 3:00 pm')
