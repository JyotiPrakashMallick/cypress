...
describe('Regression of Reminder screen touch points negative scenarios-> Single',function()
{
beforeEach(()=>{
...
it('validate the SMS message edit for special and alphanumeric characters-> For Single',function(){
    cy.log('Edit the message')
    cy.get(remindersScreenSelectors.editMessage,{force: true})
    .type('Ed!ted the 7 d@ys me$$@ge for SMS. ')
    cy.get(remindersScreenSelectors.messageText).click()
    cy.log('Save the Reminder')
    cy.get(remindersScreenSelectors.savetheReminder).click()
    cy.wait('@updateTouchPoint')
    cy.log('Validate the special character and alphanumeric text')
    cy.get(remindersScreenSelectors.friendlyReminder7Days).click()
    cy.get(remindersScreenSelectors.editMessageBox, {timeout: 5000}).scrollIntoView()
    cy.validateReminderScreenContent(remindersScreenSelectors.editMessage, 'Ed!ted the 7 d@ys me$$@ge for SMS.')
    cy.get(remindersScreenSelectors.closeMessage).click()

})


it('validate the Email message edit for special and alphanumeric characters-> For Single',function(){
    ...
    cy.get(remindersScreenSelectors.friendlyReminder7Days).click()   
    cy.get(remindersScreenSelectors.editMessageBox, {timeout: 5000}).scrollIntoView()
    cy.log('Enable the Email view to validate the message')
    cy.get(remindersScreenSelectors.enableEmailView).contains('Email').click()
    cy.log('Validate Email is enabled')
    cy.validateReminderText(remindersScreenSelectors.validateEmailEnabled,'Email')
...
it('validate the Email message edit for maximum number of characters-> For Single',function(){
    cy.log('Enable the Email view to update the message')
    cy.get(remindersScreenSelectors.enableEmailView).contains('Email').click()
    cy.log('Validate Email is enabled')
    cy.validateReminderText(remindersScreenSelectors.validateEmailEnabled,'Email')
    ...
    cy.get(remindersScreenSelectors.editEmailSubject)
    .invoke('show').type('1234567890123456789012345678901234567890123456')
