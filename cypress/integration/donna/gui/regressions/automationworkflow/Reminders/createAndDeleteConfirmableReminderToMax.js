...
/* EPIC: AUTO-446 Create and Delete a new touch point on donna's new UI */
    /*Create a touch point for confirmable reminder
...
describe('Validate if confirmable reminders can be created only upto its maximum limit of 8, and can be deleted also', () => {
    before(function () {
    ...
    it('create the confirmable reminders until its maximum limit', function () {
        cy.createMaxConfirmableReminders(remindersScreenSelectors.allTouchPointReminders)
    })

    it('try creating one more Confirmable reminder & validate if the application doesn\'t allow', function () {
        ...
        cy.get('#messageType').siblings('div').should('have.text', 'Confirmable Reminder')
        cy.get(remindersScreenSelectors.warningMsg).should('contain.text', 'There cannot be more than 8 Confirmable Reminders.')
        ...
        cy.enterDropDownValue(remindersScreenSelectors.whentoSendMessage, '15{enter}')
    ...
    it('Validate if each Confirmable reminder-touchpoint can be deleted, which were created during this test', function () {
        for (let i = 10; i < 14; i++) {
            ...
            cy.get(remindersScreenSelectors.allTouchPointTitles).should('not.contain.text',i+' Confirmable Reminder')
        }
    })
})